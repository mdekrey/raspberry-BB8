    ...
    <div>
      <svg width={size} height={size}>
        {motorZipped.map((motor, index) =>
          <g key={index} transform={`translate(${size * 0.5}, ${size * 0.5}) rotate(${motor.orientation})`}>
            ...
            {motor.gpioPower > 0
              ...
              : null}
              <text transform={`rotate(-90) translate(0, ${size * 0.2})`} textAnchor="middle">{motor.gpioPower.toFixed(1)}</text>
          </g>
        )}
      </svg>
